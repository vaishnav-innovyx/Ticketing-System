<script lang="ts">
	...
	let hasDependencies = $state(false);
	let dependencyPeople = $state<Array<{ label: string; detail: string }>>([]);
	let dependencyModules = $state<Array<{ label: string; detail: string }>>([]);
	let personLabelInput = $state('');
	let personDetailInput = $state('');
	let moduleLabelInput = $state('');
	let moduleDetailInput = $state('');

	function addDependencyPerson() {
		const label = personLabelInput.trim();
		if (!label) return;
		dependencyPeople = [...dependencyPeople, { label, detail: personDetailInput.trim() }];
		personLabelInput = '';
		personDetailInput = '';
	}

	function removeDependencyPerson(index: number) {
		dependencyPeople = dependencyPeople.filter((_, i) => i !== index);
	}

	function addDependencyModule() {
		const label = moduleLabelInput.trim();
		if (!label) return;
		dependencyModules = [...dependencyModules, { label, detail: moduleDetailInput.trim() }];
		moduleLabelInput = '';
		moduleDetailInput = '';
	}

	function removeDependencyModule(index: number) {
		dependencyModules = dependencyModules.filter((_, i) => i !== index);
	}
	...
	{:else}
		...
		<div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
			...
			>
				...
				{#if hasDependencies}
					{#each dependencyPeople as person}
						<input type="hidden" name="dependency_kind" value="person" />
						<input type="hidden" name="dependency_label" value={person.label} />
						<input type="hidden" name="dependency_detail" value={person.detail} />
					{/each}
					{#each dependencyModules as mod}
						<input type="hidden" name="dependency_kind" value="module" />
						<input type="hidden" name="dependency_label" value={mod.label} />
						<input type="hidden" name="dependency_detail" value={mod.detail} />
					{/each}
				{/if}
				...
				<!-- Section 2b: Dependencies -->
				<div class="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-lowest)] p-6 shadow-xs space-y-4">
					<div class="flex items-center justify-between pb-3 border-b border-[var(--color-border-subtle)]">
						<div class="flex items-center gap-2.5">
							<span class="material-symbols-outlined text-[var(--color-primary)] text-2xl">hub</span>
							<h2 class="text-lg font-bold text-[var(--color-on-surface)]">
								Dependencies
							</h2>
						</div>
					...
					<label class="flex items-start gap-3 cursor-pointer">
						<input
							type="checkbox"
							bind:checked={hasDependencies}
							class="mt-0.5 h-4 w-4 rounded border-[var(--color-border-subtle)] text-[var(--color-primary)] focus:ring-[var(--color-primary)]/20"
						/>
						<span class="text-body-sm text-[var(--color-on-surface)]">
							This issue involves other people or modules (e.g. a report generated by
							multiple employees, or a system this feature depends on).
						</span>
					</label>

					{#if hasDependencies}
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-[var(--color-border-subtle)]/60">
							<!-- People involved -->
							<div class="space-y-2">
								<span class="text-label-sm font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">
									People Involved
								...
								<div class="flex flex-col gap-2">
									<input
										type="text"
										bind:value={personLabelInput}
										placeholder="Name (e.g. Jane Doe)"
										class="w-full rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-lowest)] px-3 py-2 text-body-sm text-[var(--color-on-surface)] outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all placeholder:text-[var(--color-outline)]"
									/>
									<input
										type="text"
										bind:value={personDetailInput}
										placeholder="Email or note (optional)"
										class="w-full rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-lowest)] px-3 py-2 text-body-sm text-[var(--color-on-surface)] outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all placeholder:text-[var(--color-outline)]"
									/>
									<button
										type="button"
										onclick={addDependencyPerson}
										class="self-start rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-low)] px-3 py-1.5 text-label-sm font-semibold text-[var(--color-on-surface)] hover:bg-[var(--color-surface-container-high)] hover:text-[var(--color-primary)] transition-colors cursor-pointer"
									>
										Add Person
									</button>
								</div>
								{#if dependencyPeople.length > 0}
									<div class="flex flex-wrap gap-2 pt-1">
										{#each dependencyPeople as person, i}
											<div class="inline-flex items-center gap-2 rounded-lg border border-[var(--color-primary)]/40 bg-[var(--color-primary-fixed)]/20 px-3 py-1.5 text-body-sm text-[var(--color-on-surface)]">
												<span class="material-symbols-outlined text-[16px] text-[var(--color-primary)]">person</span>
												<span class="font-semibold text-xs">{person.label}</span>
												{#if person.detail}
													<span class="text-[11px] text-[var(--color-on-surface-variant)]">({person.detail})</span>
												{/if}
												<button
													type="button"
													onclick={() => removeDependencyPerson(i)}
													class="text-[var(--color-outline)] hover:text-[var(--color-error)] transition-colors ml-1 cursor-pointer"
													title="Remove"
												>
													<span class="material-symbols-outlined text-[16px]">close</span>
							...
							<div class="space-y-2">
								...
								<div class="flex flex-col gap-2">
									<input
										type="text"
										bind:value={moduleLabelInput}
										placeholder="Module or system name"
										class="w-full rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-lowest)] px-3 py-2 text-body-sm text-[var(--color-on-surface)] outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all placeholder:text-[var(--color-outline)]"
									/>
									<input
										type="text"
										bind:value={moduleDetailInput}
										placeholder="How it's involved (optional)"
										class="w-full rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-lowest)] px-3 py-2 text-body-sm text-[var(--color-on-surface)] outline-none focus:border-[var(--color-primary)] focus:ring-2 focus:ring-[var(--color-primary)]/20 transition-all placeholder:text-[var(--color-outline)]"
									/>
									<button
										type="button"
										onclick={addDependencyModule}
										class="self-start rounded-lg border border-[var(--color-border-subtle)] bg-[var(--color-surface-container-low)] px-3 py-1.5 text-label-sm font-semibold text-[var(--color-on-surface)] hover:bg-[var(--color-surface-container-high)] hover:text-[var(--color-primary)] transition-colors cursor-pointer"
									>
										Add Module
									</button>
								...
								{#if dependencyModules.length > 0}
									<div class="flex flex-wrap gap-2 pt-1">
										{#each dependencyModules as mod, i}
											<div class="inline-flex items-center gap-2 rounded-lg border border-[var(--color-primary)]/40 bg-[var(--color-primary-fixed)]/20 px-3 py-1.5 text-body-sm text-[var(--color-on-surface)]">
												<span class="material-symbols-outlined text-[16px] text-[var(--color-primary)]">apps</span>
												<span class="font-semibold text-xs">{mod.label}</span>
												{#if mod.detail}
													<span class="text-[11px] text-[var(--color-on-surface-variant)]">({mod.detail})</span>
												{/if}
												<button
													type="button"
													onclick={() => removeDependencyModule(i)}
													class="text-[var(--color-outline)] hover:text-[var(--color-error)] transition-colors ml-1 cursor-pointer"
													title="Remove"
												>
													<span class="material-symbols-outlined text-[16px]">close</span>
